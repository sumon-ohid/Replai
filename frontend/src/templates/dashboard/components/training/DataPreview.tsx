...
export function DataPreview({
  ...
  const [fileData, setPdfData] = React.useState<PDFData>({
    files: [],
    totalCharCount: 0,
    fileBody: "",
  });
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const response = await axios.get(
          `${apiBaseUrl}/api/data/get-training-data`,
          {
        ...
        if (response.status === 200) {
          ...
          if (typeof data === "object" && data !== null && "textData" in data) {
            ...
            setTextData({
              entries: textSamples,
              charCount:
                typeof data.textData === "string" ? data.textData.length : 0,
            });
          }
          ...
          if (typeof data === "object" && data !== null && "fileData" in data) {
            ...
            setPdfData({
              // file data is a string, show only first 100 characters
              files:
                typeof data.fileData === "string"
                  ? [{ name: "PDF Document", charCount: data.fileData.length }]
                  : [],
              totalCharCount:
                typeof data.fileData === "string" ? data.fileData.length : 0,
              fileBody:
                typeof data.fileData === "string"
                  ? data.fileData.substring(0, 100)
                  : "",
            });
          }
          ...
          if (typeof data === "object" && data !== null && "webData" in data) {
            ...
            setWebsiteData({
              urls,
              totalCharCount:
                typeof data.webData === "string" ? data.webData.length : 0,
            });
          }
        }
      } catch (error) {
  ...
  const calculateDataStats = () => {
    ...
    const totalDocuments =
      ...
      (selectedSources.includes("pdf") ? fileData.files.length : 0) +
    ...
    const totalCharCount =
      ...
      (selectedSources.includes("pdf") ? fileData.totalCharCount : 0) +
              ...
              {textData.entries.length > 0 ? (
                <>
                  <Typography
                    ...
                    sx={{ mb: 1 }}
                  >
                    <strong>{textData.entries.length}</strong> text entries
                    detected
                  </Typography>
              ...
              {fileData.files.length > 0 ? (
                <>
                  <Typography
                    ...
                    sx={{ mb: 1 }}
                  >
                    <strong>{fileData.files.length}</strong> PDF files processed
                  </Typography>
                        ...
                        <ListItemText
                          ...
                          secondary={
                            file.pages
                              ? `${
                                  file.pages
                                } pages • ${file.charCount.toLocaleString()} characters`
              ...
              {websiteData.urls.length > 0 ? (
                <>
                  <Typography
                    ...
                    sx={{ mb: 1 }}
                  >
                    <strong>{websiteData.urls.length}</strong> websites crawled
                    for content
                  </Typography>
                        ...
                        <ListItemText
                          ...
                          secondary={
                            <Tooltip title={urlData.url} placement="bottom">
                              ...
                              >
                                {urlData.url} •{" "}
                                {urlData.charCount.toLocaleString()} characters
                              </Typography>
                            </Tooltip>
                          }
                          primaryTypographyProps={{
    ...
    >
      <Box component={motion.div} variants={itemVariants}>
        <Typography
          variant="h5"
          fontWeight={700}
          sx={{ mb: 3, fontSize: isMobile ? "1.25rem" : "1.5rem" }}
        >
            ...
            <CardContent sx={{ p: isMobile ? 2 : 3 }}>
              ...
              >
                ...
                <Box component="span" sx={{ fontWeight: 600, mx: 0.5 }}>
                  {dataStats.totalCharCount.toLocaleString()}
                </Box>
                characters will be processed during training to create your
                custom AI assistant.
              </Typography>
            </CardContent>
          </Card>
            ...
            >
              ...
              <Box>
                ...
                <Typography variant="body2" color="text.secondary">
                  Your data has been processed and is ready for training. Click
