...
router.get("/status", auth, async (req, res) => {
  try {
    ...
    const isConnected = Boolean(
      user.googleAuth?.accessToken && user.googleAuth?.refreshToken
    );
    ...
    logger.error("Failed to check calendar connection status", {
      error,
      userId: req.user.id,
    });
    res
      .status(500)
      .json({ error: "Failed to check Google Calendar connection" });
  }
});
...
router.get("/auth/google", auth, async (req, res) => {
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to generate Google authorization URL" });
  }
});
        ...
        <body>
          <div class="card">
            <div class="success-icon">
              <dotlottie-player src="https://lottie.host/8f4833b2-27df-4a69-8b5b-a849ff7edbd3/RsPPBZo7Cy.lottie" background="transparent" speed="1" style="width: 300px; height: 300px" autoplay></dotlottie-player>
            </div>
...
router.get("/events", auth, async (req, res) => {
  try {
    ...
    if (error.message === "NOT_CONNECTED") {
      return res
        .status(401)
        .json({ error: "Not connected to Google Calendar" });
    }
  ...
  try {
    ...
    if (req.body.enableMeet) {
      eventData.conferenceData = {
        createRequest: {
          requestId: `${Date.now()}-${userId}-${Math.random()
            .toString(36)
            .substring(2, 7)}`,
    ...
    if (req.body.allDay) {
      ...
      if (!req.body.end) {
        const endTime = new Date(
          new Date(req.body.start).getTime() + 60 * 60 * 1000
        );
...
router.put("/events/:id", auth, async (req, res) => {
  try {
    ...
    const eventData = {
      ...
      colorId:
        "colorId" in req.body ? req.body.colorId : existingEvent.data.colorId,
      start: {},
      end: {},
    };
    ...
    const isAllDay =
      "allDay" in req.body
        ? req.body.allDay
        : Boolean(existingEvent.data.start.date);
    ...
    if (isAllDay) {
      ...
      eventData.start.dateTime =
        req.body.start || existingEvent.data.start.dateTime;
      ...
      if (req.body.end) {
        ...
        const endTime = new Date(
          new Date(eventData.start.dateTime).getTime() + 60 * 60 * 1000
        );
        eventData.end.dateTime = endTime.toISOString();
      }
      ...
      if (existingEvent.data.start.timeZone) {
        eventData.start.timeZone =
          req.body.timeZone || existingEvent.data.start.timeZone;
        eventData.end.timeZone =
          req.body.timeZone || existingEvent.data.end.timeZone;
      }
    }
