...
import { getConnection, updateConnectionConfig } from '../managers/connectionManager.js';
...
export const toggleAutoReply = async (req, res) => {
  try {
    ...
    if (!connectedEmail) {
      return res.status(404).json({ error: 'Email not found in connected accounts' });
    }
    ...
    const connectionUpdated = await updateConnectionConfig(userId, email, { mode });
    ...
    if (connection) {
      ...
      if (enabled && connection.checkForNewEmails) {
        try {
          ...
          console.error(`Error checking for new emails on sync toggle: ${syncError}`);
        }
      }
    }
...
export const updateEmailMode = async (req, res) => {
  try {
    ...
    if (!['draft', 'normal', 'auto-reply'].includes(mode)) {
      return res.status(400).json({ error: 'Invalid mode. Must be draft, normal, or auto-reply' });
    }
...
export const refreshEmailSync = async (req, res) => {
  try {
    ...
    if (!connection) {
      return res.status(404).json({ error: 'No active connection found for this email' });
    }
    ...
    try {
      if (connection.checkForNewEmails) {
        await connection.checkForNewEmails();
        ...
        syncError = 'Connection does not support manual sync';
      }
    } catch (error) {
